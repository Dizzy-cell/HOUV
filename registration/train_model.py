...
def train():
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            src_t, src_f, tgt, src_t_len, src_f_len, tgt_len, label_t, label_f, xyz_t, xyz_f, xyz_gt, grid_t, grid_f, grid_gt =  data

            src = torch.cat([src_t, src_f], dim = 0)
            tgt = torch.cat([tgt, tgt], dim = 0)
            src_len = torch.cat([src_t_len, src_f_len], dim = 0)
            label = torch.cat([label_t, label_f], dim = 0).long().cuda()

            xyz = torch.cat([xyz_t, xyz_f], dim = 0).float().cuda()
            xyz_gt = torch.cat([xyz_gt, xyz_gt], dim = 0).float().cuda()
            ...
            total_train_loss, acc = net(src, src_len, label)
            total_train_loss.backward(torch.squeeze(torch.ones(torch.cuda.device_count())).cuda())
            optimizer.step()

            train_loss_meter['RotE'].update(acc)
            train_loss_meter['transE'].update(acc)
