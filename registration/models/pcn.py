...
from model_utils_completion import gen_grid_up, calc_emd, calc_cd
...
class PCN_Transformer(nn.Module):
    def __init__(self, output_size=3):
        super(PCN_Transformer, self).__init__()
...
class PCN_encoder_label(nn.Module):
    def __init__(self, output_size=1024):
        super(PCN_encoder_label, self).__init__()
        self.conv1 = nn.Conv1d(3, 512, 1)
        self.conv2 = nn.Conv1d(1280, 512, 1)
    ...
    def forward(self, x, label = None):
        batch_size, _, num_points = x.size()
        x = F.relu(self.conv1(x))
        em = self.embedding(label)
        global_feature, _ = torch.max(x, 2)

        global_feature = torch.cat((em, global_feature), dim = 1)
        ...
        x = F.relu(self.conv2(x))
        x = F.relu(self.conv3(x))
        voxel_feature = x.transpose(1,2).contiguous()
        x = self.conv4(x)
        global_feature, _ = torch.max(x, 2)
        
        return global_feature.view(batch_size, -1), voxel_feature
