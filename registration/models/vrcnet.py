...
from model_utils_completion import *
from models.pcn import PCN_encoder, PCN_Transformer
...
class Model(nn.Module):
    def __init__(self, args, size_z=128, global_feature_size=3):
        ...
        self.encoder = PCN_Transformer(output_size=global_feature_size)

        self.loss_mse =  loss = nn.MSELoss()
    ...
    def forward(self,
        ...
        if prefix == "train":
            ...
            mse = self.loss_mse(fine, gt)
            total_train_loss = loss4_t.mean() + loss4.mean() + mse
            return total_train_loss, loss4, mse, fine
