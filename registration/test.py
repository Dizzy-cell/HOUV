...
def test():
    ...
    dataset_test = MVP_RG_rotated(prefix="test", args=args)
    ...
    result_list = []
    for _, data in tqdm(enumerate(dataloader_test)):
        src, tgt, _ = data
        src = src.float().cuda()
        tgt = tgt.float().cuda()
        ...
        #     kernel = 32
        #     src_b = src[j:j+1].expand((kernel, 2048, 3))
        #     tgt_b = src[j:j+1].expand((kernel, 2048, 3))

        #     result = getPredict_test_keba(src_b, tgt_b)
