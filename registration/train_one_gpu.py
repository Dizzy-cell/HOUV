...
from train_utils import getPredict, getPredict_cd, getPredict_cd_keba, getPredict_cd_keba_module
...
def train():
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            src, tgt, src_rotated,  tgt_rotated, transform, match_level, rot_level, pose1, pose2, angle_t ,label, src_vox, tgt_vox, _, _ = data
            src = src.float().cuda()
            tgt = tgt.float().cuda()
            ...
            for j in range(src_rotated.shape[0]):
                kenrnel = 32
                src_b = src_rotated[j:j+1].expand((kenrnel, 2048, 3))
                tgt_b = tgt_rotated[j:j+1].expand((kenrnel, 2048, 3))
                transform_b = transform[j:j+1].expand((kenrnel, 4, 4))
                src_ob= src[j:j+1].expand((kenrnel, 2048, 3))
                tgt_ob = tgt[j:j+1].expand((kenrnel, 2048, 3))
                angle_t = angle_t[j:j+1].expand((kenrnel,1))
                r_err, t_err = getPredict_cd_keba_module(src_b, tgt_b, transform_b, src_ob, tgt_ob, angle_t)
                train_loss_meter['RotE'].update(r_err.item())
                train_loss_meter['transE'].update(t_err.item())
