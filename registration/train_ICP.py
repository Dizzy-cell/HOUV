...
def train():
    ...
    dataloader = dataloader_test      #val test dataset
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            j = 0
            plot_grid_pcd([src[j].detach().cpu().numpy(), src[j].detach().cpu().numpy(),src[j].detach().cpu().numpy(), src[j].detach().cpu().numpy(), src[j].detach().cpu().numpy()], shape = [1,5], save_path ='Grads/object_{}.png'.format(i), title = '')
