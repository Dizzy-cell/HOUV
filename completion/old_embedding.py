...
def train():
    ...
    # dataset = MVP_CP_voxel_point(prefix="train")
    # dataset_test = MVP_CP_voxel_point(prefix="val")

    dataset = MVP_CP_choose(prefix='train', idx = 0)
    dataset_test = MVP_CP_choose(prefix="val", idx = 0)
    
    size = dataset_test.__len__()
    print(size)
    for i in range(size):
        data_a = dataset_test.__getitem__(i)
        mi = 9999
        idx = -1
        mf1 = -1
        for j in range(i+1,size):
            data_b = dataset_test.__getitem__(j)
            
            # f1 bigger, cd_p and cd_t smaller
            cd_p, cd_t, f1 = calc_cd(data_a[-2].reshape(-1, 2048, 3).to(device), data_b[-2].reshape(-1,2048,3).to(device), calc_f1=True)
            if cd_p + cd_t < mi:
                mi = cd_p + cd_t
                mf1 = f1
                idx = j
            #print(i, j, cd_p, cd_t, f1)
        com_cd_p, com_cd_t, com_f1 = calc_cd(data_a[-2].reshape(-1, 2048, 3).to(device), data_a[-1].reshape(-1,2048,3).to(device), calc_f1=True)
        

        print("Number id: {} Similar id: {} CD:({})/({}) F1({})/({}) Equal:{}".format(i, idx, mi, com_cd_p + com_cd_t, mf1, com_f1, i // 26 == idx // 26))
    ...
    #     plot_combine_pcd(points, points_vis, 'visuilation/{}_combine_choose.png'.format(i))
