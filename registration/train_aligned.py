...
from dataset import MVP_RG, MVP_RG_rotated, MVP_RG_Aligned
...
from train_utils import getPredict, getPredict_cd, getPredict_cd_keba, getPredict_cd_keba_module, getPredict_cd_keba_v2, getPredict_cd_keba_v3, getPredict_cd_vox
...
def train():
    ...
    dataset = MVP_RG_Aligned(prefix="train", args=args)
    dataset_test = MVP_RG_Aligned(prefix="val", args=args)
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            src, tgt, src_rotated,  tgt_rotated, transform, match_level, rot_level, pose1, pose2, angle_t ,label, src_vox, tgt_vox, src_vox_len, tgt_vox_len = data
            ...
            # src_vox = src_vox.float().cuda()
            # tgt_vox = tgt_vox.float().cuda()
            # src_vox_len = src_vox_len.long().cuda()
            # tgt_vox_len = tgt_vox_len.long().cuda()


            total_train_loss, loss4, loss4_t, fine = net(src_rotated.transpose(2,1).contiguous(), src)
            total_train_loss.backward(torch.squeeze(torch.ones(torch.cuda.device_count())).cuda())
            optimizer.step()

            # train_loss_meter['RotE'].update(loss4.mean().item())
            # train_loss_meter['transE'].update(loss4_t.mean().item())

            a = fine.detach()
            r_err, t_err, ans = getPredict_cd_keba_v3(a[:8], src_rotated, pose1, src, tgt, angle_t, label)
            ...
            if i % 100 == 0:
                logging.info('cd_t:{} cd_p:{}'.format(train_loss_meter['RotE'].avg, train_loss_meter['transE'].avg))
...
def val(net, curr_epoch_num, val_loss_meters, val_split_loss_meters, dataloader_test, best_epoch_losses, rmse_thresh=0.1):
    ...
    with torch.no_grad():
        for _, data in enumerate(dataloader_test):
            ...
            src, tgt, src_rotated,  tgt_rotated, transform, match_level, rot_level, pose1, pose2 = data
            ...
            _, r_err, t_err, rmse, mse = net(src, tgt, prefix="val")
