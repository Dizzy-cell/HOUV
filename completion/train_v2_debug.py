...
from vis_utils import visulation_max_loss, visulation_predict
...
def train():
    ...
    args.batch_size = args.every_batch_size * torch.cuda.device_count()
    ...
    if args.optimizer == 'Adagrad':
        optimizer = optimizer(net.module.parameters(),
                              lr=lr,
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            out2, loss2, net_loss = net(inputs, gt, label=label)

            loss_max = visulation_max_loss(inputs.detach().cpu().numpy(), gt.detach().cpu().numpy(), 
                                            out2.detach().cpu().numpy(), loss2.detach().cpu().numpy(), loss_max, save_path='visuilation/Hard_sample_{}.png'.format(i))
            if i % 10 == 0:
                visulation_predict(inputs.detach().cpu().numpy(), gt.detach().cpu().numpy(), 
                                            out2.detach().cpu().numpy(), save_path = 'visuilation/Sample_{}.png'.format(i))

            train_loss_meter.update(net_loss.mean().item())
            net_loss.backward(
                torch.squeeze(torch.ones(
            ...
            if i % args.step_interval_to_print == 0:
                logging.info(
                    exp_name +
                    ' train [%d: %d/%d]  loss_type: %s, fine_loss: %f total_loss: %f lr: %f'
                    % (epoch, i, len(dataset) / args.batch_size, args.loss,
                       loss2.mean().item(), net_loss.mean().item(), lr) +
                    ' alpha: ' + str(alpha))
