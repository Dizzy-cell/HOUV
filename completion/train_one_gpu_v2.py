...
import sys
import argparse
from dataset import MVP_CP, MVP_CP_choose
...
def train():
    ...
    #     # embed()
    #     cd_p, cd_c, f1 = calc_cd(a_p.reshape(1,2048,3).cuda(), a_c.reshape(1,2048,3).cuda(), calc_f1=True)
    ...
    #         b_p, b_c, f2 = calc_cd(a_p.unsqueeze(0).cuda(), data_b[1].unsqueeze(0).cuda(), calc_f1 = True)
    #         if f2 > mi:
    #             mi = f2
    #             bp = b_p
    #             bc = b_c
    ...
    for epoch in range(args.start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            out2, loss2, net_loss = net(inputs, gt, label, alpha=alpha)

            train_loss_meter.update(net_loss.mean().item())
            net_loss.backward(
                torch.squeeze(torch.ones(torch.cuda.device_count())).cuda())
            optimizer.step()
