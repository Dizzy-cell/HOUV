...
def test():
    dataset_test = MVP_CP_EX(prefix="test")
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(dataloader_test):
            ...
            label, inputs_cpu, _, voxels, coords = data

            inputs = inputs_cpu.float().cuda()
            label = label.long().cuda()
            inputs = inputs.transpose(2, 1).contiguous()

            result_dict = net(inputs, gt = inputs, prefix="test", label = label, voxels = voxels, coords = coords)
...
if __name__ == "__main__":
    ...
    logging.basicConfig(level=logging.INFO, handlers=[logging.FileHandler(os.path.join(log_dir, 'test.log')), logging.StreamHandler(sys.stdout)])
